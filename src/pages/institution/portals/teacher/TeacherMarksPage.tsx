...
export default function TeacherMarksPage() {
  ...
  const { exams, loading: examLoading } = useExams();
  ...
  const { examSubjects } = useExamSubjects(selExamId);
  const selExamSubject = examSubjects.find((es) => es.subject_id === selSubjectId);
  const selExam = exams.find((e) => e.id === selExamId);

  // useUpsertStudentMark takes examSubjectId as argument
  const upsertMark = useUpsertStudentMark(selExamSubject?.id ?? "");
  ...
  const getMark = (studentId: string, field: "theory" | "practical" | "viva") => {
    ...
    return String(existing.viva_marks ?? "");
  };
    ...
    for (const s of students) {
      try {
        await upsertMark.mutateAsync({
          ...
          theory_marks:    parseFloat(getMark(s.user_id, "theory"))    || 0,
          practical_marks: parseFloat(getMark(s.user_id, "practical")) || 0,
          viva_marks:      parseFloat(getMark(s.user_id, "viva"))      || 0,
        });
        saved++;
      } catch { /* continue */ }
    }
    ...
    <div className="p-6 space-y-6">
      ...
      <Card className="border-border/50">
        <CardContent className="p-4">
          ...
          {selExamSubject && (
            <div className="mt-3 p-3 rounded-lg bg-secondary/50 flex gap-4 flex-wrap text-xs text-muted-foreground">
              <span><BookOpen className="inline h-3.5 w-3.5 mr-1" />Total: <strong>{selExamSubject.total_marks}</strong></span>
              <span>Passing: <strong>{selExamSubject.passing_marks}</strong></span>
              <span>Theory: <strong>{selExamSubject.theory_weightage}%</strong></span>
              <span>Practical: <strong>{selExamSubject.practical_weightage}%</strong></span>
              <span>Viva: <strong>{selExamSubject.viva_weightage}%</strong></span>
            </div>
          )}
        </CardContent>
      </Card>
            ...
            <CardHeader className="pb-3">
              <div className="flex items-center justify-between">
                <CardTitle className="text-sm flex items-center gap-2">
                  ...
                  {selExam?.name} · {subjects.find((s) => s.id === selSubjectId)?.name}
                </CardTitle>
                  ...
                  {students.map((s, i) => {
                    ...
                    return (
                      <TableRow key={s.id}>
                        ...
                        {(["theory","practical","viva"] as const).map((field) => (
                          <TableCell key={field}>
                            <Input type="number" min={0} max={selExamSubject?.total_marks ?? 100}
                              value={getMark(s.user_id, field)}
                              onChange={(e) => setMark(s.user_id, field, e.target.value)}
